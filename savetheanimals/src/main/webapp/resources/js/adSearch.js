...
function Search(id, subid) {
	
	/*var sub = SubCity(subid);
	var mainCity = document.getElementById(id);
	var main = mainCity.options[mainCity.selectedIndex].text;
	var ani_occur_address = ""

	if(main!='전체'){
		if(sub!='전체'){
			ani_occur_address = main+" "+sub;
		}
		else{
			ani_occur_address = main;
		}
	}
		main주소 value를 바꿔서 값 보내기
		mainCity.options[mainCity.selectedIndex].value = ani_occur_address;
		
		alert(mainCity.options[mainCity.selectedIndex].value);*/
}
